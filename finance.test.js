...
function fixture(t,hosted){
 ...
 if(hosted){
  const document=api.snapshot(),state=stateFromSnapshot(document,Buffer.from(document.key,'base64'));state.sessions=api.db.prepare('SELECT * FROM sessions').all();api.close();
  ...
  origin='https://pto.test';handle=r=>handleCloudApi(r,{DB,PTO_SECURITY_KEY:document.key});snapshot=()=>{const s=JSON.parse(sql.prepare('SELECT document FROM pto_state').get().document);return {...document,tables:{...document.tables,users:s.users,workspace:[s.workspace]}};};
