...
let mode, data, loadError = '', cloud=null, accountEmail='', saving=false;
...
async function start() {
  ...
  try {
    ...
    if(response.status===401) {
      $('#app').innerHTML='<div class="startup"><h1>Bandbook</h1><p>Sign in to open your ledger.</p><a class="button primary" href="/signin-with-chatgpt?return_to=%2F" target="_top">Sign in with ChatGPT</a></div>';
      return;
    }
    if(!response.ok) throw new Error('Could not open your ledger.');
    const session=await response.json();
    cloud=new CloudLedger();accountEmail=session.email;mode='cloud';loadError='';
    data=validateBackup(await cloud.load());clearDraft();render();
  } catch {
    $('#app').innerHTML='<div class="startup"><h1>Unable to open the ledger</h1><p>Check your connection and sign-in, then try again.</p><button class="button primary" data-action="reload">Try again</button><a class="text-button" href="/signin-with-chatgpt?return_to=%2F" target="_top">Sign in</a></div>';
  }
}
start();
