...
export async function handleApi(request, env) {
  ...
  if(request.headers.get('origin')!==url.origin || request.headers.get('x-bandbook-request')!=='1') return json({error:'This request must come from Bandbook.'},403);
...
export default {
  async fetch(request,env) {
    ...
    try {
      ...
      return secured;
    } catch { return json({error:'The ledger is temporarily unavailable. Try again shortly.'},503); }
  },
};
